...
private ["_lastSimulationCheck","_lastCleanupCheck","_lastQueueCheck","_lastHelicrashCheck","_lastTimeoutCheck","_lastDebugCheck","_helicrashCount","_serverRestartInterval","_overwriteHardCodeRestart"];
...
_helicrashCount = 5;
_overwriteHardCodeRestart = getNumber (configFile >> "CfgBreakingPointServerSettings" >> "ServerRestartSettings" >> "overwriteExtensionCall");
_serverRestartInterval = (getNumber (configFile >> "CfgBreakingPointServerSettings" >> "ServerRestartSettings" >> "restartInterval")) * 60;

diag_log format ["Server restart mode is %1 and will restart in %2 seconds",_overwriteHardCodeRestart,_serverRestartInterval];
...
waitUntil
{
	...
	if ((diag_tickTime - _lastCleanupCheck) > CLEANUP_CHECK_INT) then 
	{
		...
		if !(_overwriteHardCodeRestart isEqualTo 1) then
		{	
			if (!shutdown) then
			{
		...
		{
			if (time > (_serverRestartInterval -300)) then
			{
				["serverMgr: Restart Cleanup and Sync (2 Hours 55 Mins)"] call BP_fnc_debugConsoleFormat;
				[[], BPServer_fnc_cleanupRestart] execFSM "\breakingpoint_code\system\call.fsm";
			};
		};	
	};
